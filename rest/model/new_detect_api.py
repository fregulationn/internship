...
def main(args):
    ...
    with tf.Graph().as_default():
        ...
        output_graph_path = 'D:/Z/model/mtcnn_graph.pb'
...
def create_mtcnn(sess, model_path):
    ...
    pnet_input = sess.graph.get_tensor_by_name("pnet/input:0")
    pnet_conv = sess.graph.get_tensor_by_name("pnet/conv4-2/BiasAdd:0")
    pnet_prob = sess.graph.get_tensor_by_name("pnet/prob1:0")

    rnet_input = sess.graph.get_tensor_by_name("rnet/input:0")
    rnet_conv = sess.graph.get_tensor_by_name("rnet/conv5-2/conv5-2:0")
    rnet_prob = sess.graph.get_tensor_by_name("rnet/prob1:0")

    onet_input = sess.graph.get_tensor_by_name("onet/input:0")
    onet_conv = sess.graph.get_tensor_by_name("onet/conv6-2/conv6-2:0")
    onet_conv1 = sess.graph.get_tensor_by_name("onet/conv6-3/conv6-3:0")
    onet_prob = sess.graph.get_tensor_by_name("onet/prob1:0")



    pnet_fun = lambda img: sess.run((pnet_conv, pnet_prob), feed_dict={pnet_input: img})
    rnet_fun = lambda img: sess.run((rnet_conv, rnet_prob), feed_dict={rnet_input: img})
    onet_fun = lambda img: sess.run((onet_conv, onet_conv1, onet_prob),
...
def detect_face(img, minsize, pnet, rnet, onet, threshold, factor):
    ...
    for scale in scales:
        ...
        im_data = (im_data.astype(np.float32) - 127.5) * 0.0078125
